 ...
 */
export interface MotionExecuteDialogHandle {
  show: () => void;
}

interface MotionExecuteDialogProps {
 ...
 */
const TITLE = "Motion Execute";
const SUBTITLE = "- ユニットの行動 -";
 ...
 */
const MotionExecuteDialog = forwardRef<MotionExecuteDialogHandle, MotionExecuteDialogProps>(
  /**
   * ここに書いている MotionExecuteDialog は、forwardRef に渡している描画関数の名前。
   * 自分で直接呼び出しているわけではなく、<MotionExecuteDialog ... /> と書かれたときに
   * React がこの関数を呼んで描画する。
   */
  function MotionExecuteDialog({ turn, onAnimationEnd }, ref) {
        ...
        >
          <div className="flex w-full items-center justify-between gap-4 text-white">
            ...
            <div className="flex-1 text-center">
              <p
                className="whitespace-nowrap text-[clamp(2rem,5vw,4.2rem)] italic text-orange-400"
                style={{ fontFamily: "michroma, sans-serif" }}
              >
                {TITLE}
              </p>
              <p className="text-[clamp(1.6rem,2vw,4.2rem)] text-orange-400">
                {SUBTITLE}
              </p>
              <p
...
MotionExecuteDialog.displayName = "MotionExecuteDialog";

export default MotionExecuteDialog;
