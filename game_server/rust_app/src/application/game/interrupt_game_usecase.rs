...
mod tests {
    ...
    async fn execute_waiting_player_interrupts_then_deletes_connection() {
        ...
        let game_repository = Arc::new(MockGameRepository::default());
        let websocket_sender = Arc::new(MockWebSocketSender::default());
        ...
        assert_eq!(game_repository.updated_games.lock().unwrap().len(), 1);
        assert!(game_repository.updated_games.lock().unwrap()[0]
            .game_state()
            .is_completed());
        assert_eq!(
            connection_repository
