...
```bash
game_server$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
game_server$ sam local start-api
game_server$ curl http://localhost:3000/
```
...
```bash
game_server$ sam logs -n HelloWorldFunction --stack-name game_server --tail
```
